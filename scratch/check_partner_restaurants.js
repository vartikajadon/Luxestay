...
async function testPR() {
    const { data, error } = await supabase.from('partner_restaurants').select('*');
    if (error) {
        console.log("partner_restaurants error:", error.message);
    } else {
        console.log("partner_restaurants rows:", JSON.stringify(data, null, 2));
    }
}
