...
async function listBreakfast() {
    const { data, error } = await supabase.from('menu_items').select('id, name, image_url, category').eq('category', 'Breakfast');
    if (error) {
        console.error("Error:", error);
    } else {
        console.log("Breakfast items in DB:", data);
    }
}
